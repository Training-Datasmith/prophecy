...
class CallCenterSpec extends ObjectBehavior
{
    ...
    public function it_records_the_error_when_stub_has_got_unexpected_method_calls(
        $objectProphecy,
        MethodProphecy $method,
        ArgumentsWildcard $arguments
    ) {
        ...
        $arguments->getTokens()->willReturn([]);

        $objectProphecy->getMethodProphecies()->willReturn(['getName' => [$method]]);
        $objectProphecy->getMethodProphecies('getName')->willReturn([$method]);
        $objectProphecy->getMethodProphecies('method1')->willReturn([]);
        $objectProphecy->reveal()->willReturn(new \stdClass());

        $this->shouldNotThrow('Prophecy\Exception\Call\UnexpectedCallException')
             ->duringMakeCall($objectProphecy, 'method1', []);
